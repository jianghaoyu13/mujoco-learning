...
class CustomViewer:
    ...
    def initGlfw(self, width, height):
        if not self.has_inited_glfw:
            ...
            self.glfw_window = glfw.create_window(
                width, height, "Offscreen", None, None
            )
    ...
    def addVisuGeom(
        self,
        geoms_pos: np.ndarray,
        geoms_type: list,
        geoms_size: np.ndarray,
        geoms_rgba: np.ndarray,
    ):
    ...
    def addObstacles(
        ...
        if worldbody is None:
            raise ValueError(
                "原始 XML 中未找到 <worldbody> 标签，请检查 MuJoCo 模型格式"
            )
        ...
        for i in range(self.num_obstacles):
            ...
            obstacle_geom.set(
                "pos", f"{pos[0]:.3f} {pos[1]:.3f} {pos[2]:.3f}"
            )  # 位置（保留3位小数）
            ...
            obstacle_geom.set(
                "rgba", f"{rgba[0]:.3f} {rgba[1]:.3f} {rgba[2]:.3f} {rgba[3]:.3f}"
            )
    ...
    def getBodyIdsByName(self):
        ...
        for body_id in range(self.model.nbody):
            ...
            # 获取父 body ID
            self.model.body_parentid[body_id]
